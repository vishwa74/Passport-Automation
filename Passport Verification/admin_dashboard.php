...
<header>
    ...
    <p>Passport Administration Office</p>
    <div class="nav-right">
...
$sql = "SELECT * FROM passport_apply WHERE status='Verified by RAO' ORDER BY appl_no LIMIT $current_index, 1";
...
if ($result->num_rows > 0) {
    ...
    // Previous button
    if ($current_index > 0) {
        echo "<a href='admin_dashboard.php?action=previous&index=" . ($current_index - 1) . "'><button>Previous</button></a>";
    }
    // Next button
    if ($result->num_rows > 0) {
        echo "<a href='admin_dashboard.php?action=next&index=" . ($current_index + 1) . "'><button>Next</button></a>";
    }
    // Verify button
    echo "<a href='admin_dashboard.php?action=verify&appl_no=" . $row["appl_no"] . "'><button>Verify Application</button></a>";
    // Reject button
    echo "<a href='admin_dashboard.php?action=reject&appl_no=" . $row["appl_no"] . "'><button>Reject Application</button></a>";
    echo "</div>";
    echo "</div>";
} else {
...
if (isset($_GET['action']) && ($_GET['action'] == 'verify' || $_GET['action'] == 'reject') && isset($_GET['appl_no'])) {
    ...
    $status = ($action == 'verify') ? 'Verified by Admin' : 'Rejected by Admin';
